...
class WOFHumanPlayer(WOFPlayer):
    def getMove(self,category, obscuredPhrase, guessed):
        ...
        print("")
        g = input("Guess a letter, phrase, or type 'exit' or 'pass': ")
        return g
#part C
class WOFComputerPlayer(WOFPlayer):
    ...
    def getPossibleLetters(self,guessed):
        to_guess = [l for l in LETTERS if l not in guessed ]
        if self.prizeMoney < VOWEL_COST:
            to_guess= list(filter(lambda l: l not in VOWELS ,to_guess ))
        return to_guess

    def getMove(self,category, obscuredPhrase, guessed):
        guess_lst = self.getPossibleLetters(guessed)
        if len(guess_lst)<1 :
            return "pass"
        else:
            move = self.smartCoinFlip()
            if move :
                poss_mostfreq = [l for l in self.SORTED_FREQUENCIES if l in guess_lst]
                return poss_mostfreq[-1]
            else:
                return random.choice(guess_lst)
